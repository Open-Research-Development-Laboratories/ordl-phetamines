# Fleet Middle-Man Flow (ordlctl Chat First)

This is the standard operating flow for your crew:

1. Workers generate their reports.
2. Reports are dumped into ordlctl chat as **UNREVIEWED WORKER DUMP**.
3. Human-in-the-middle reviews/edits in chat.
4. You can request rework with reviewer feedback.
5. Revised worker reports are restaged into the same ordlctl chat pipeline.
...
- calls `stage-worker-reports-to-gateway-chat.ps1`
- stages latest worker handoff files into the active ordlctl session
- enters a loop:
  - review in chat
