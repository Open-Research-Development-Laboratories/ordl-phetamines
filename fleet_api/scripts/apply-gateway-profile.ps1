...
function Set-Cfg {
  ...
  if ($Preview) {
    Write-Output ("[dry-run] ordlctl config set {0} {1}" -f $Path, $wireValue)
    return
  }

  & ordlctl config set $Path $wireValue | Out-Null
}
...
if (-not $DryRun) {
  & ordlctl config validate | Out-Null
}
...
$summary = [ordered]@{
  ...
  note = "Restart gateway after apply: ordlctl gateway stop ; ordlctl gateway run --bind lan"
}
