...
config_path="${HOME}/.ordlctl/ordlctl.json"
...
if [[ ! -f "${config_path}" ]]; then
  echo "ordlctl config not found at ${config_path}" >&2
  exit 1
fi

get_plugin_ids() {
  ordlctl plugins list --json | python3 - <<'PY'
import json, sys
...
resolve_bundled_dir() {
  if [[ -n "${ordlctl_BUNDLED_PLUGINS_DIR:-}" && -d "${ordlctl_BUNDLED_PLUGINS_DIR}" ]]; then
    printf '%s\n' "${ordlctl_BUNDLED_PLUGINS_DIR}"
    return 0
  fi

  local shim_dir
  shim_dir="$(cd "$(dirname "$(command -v ordlctl)")" && pwd)"
  local candidates=(
    "${shim_dir}/node_modules/ordlctl/extensions"
    "/usr/local/lib/node_modules/ordlctl/extensions"
    "/usr/lib/node_modules/ordlctl/extensions"
  )
  ...
  if command -v npm >/dev/null 2>&1; then
    ...
    if [[ -n "${npm_root}" ]]; then
      candidates+=("${npm_root}/ordlctl/extensions")
    fi
  fi
...
ordlctl config validate --json >/dev/null
