...
apply_cfg() {
  ...
  if [[ "${DRY_RUN}" == "1" ]]; then
    echo "[dry-run] openclaw config set ${path} ${value}"
    return 0
  fi
...
cat <<EOF
{
  ...
  "note": "Restart gateway after apply: openclaw gateway stop ; OPENCLAW_SKIP_GMAIL_WATCHER=1 openclaw gateway run --bind loopback"
}
EOF
