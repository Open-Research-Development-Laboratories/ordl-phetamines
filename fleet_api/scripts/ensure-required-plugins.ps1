...
if (-not (Get-Command ordlctl -ErrorAction SilentlyContinue)) {
  throw "ordlctl CLI was not found in PATH."
}

$ConfigPath = Join-Path $HOME ".ordlctl\\ordlctl.json"
if (-not (Test-Path $ConfigPath)) {
  throw "ordlctl config not found at $ConfigPath"
}

function Get-PluginIds {
  try {
    $raw = & ordlctl plugins list --json
...
function Resolve-BundledPluginsDir {
  if ($env:ordlctl_BUNDLED_PLUGINS_DIR -and (Test-Path $env:ordlctl_BUNDLED_PLUGINS_DIR)) {
    return (Resolve-Path $env:ordlctl_BUNDLED_PLUGINS_DIR).Path
  }

  $cmdSource = (Get-Command ordlctl).Source
  $shimDir = Split-Path $cmdSource -Parent
  $candidates = @(
    (Join-Path $shimDir "node_modules\\ordlctl\\extensions"),
    (Join-Path $env:APPDATA "npm\\node_modules\\ordlctl\\extensions"),
    (Join-Path $env:ProgramFiles "ordlctl\\resources\\extensions")
  )
...
if (-not $SkipValidate) {
  & ordlctl config validate --json | Out-Null
}
...
$summary = [ordered]@{
  ...
  restart_cmd = "ordlctl gateway stop; ordlctl gateway run --bind loopback"
}
