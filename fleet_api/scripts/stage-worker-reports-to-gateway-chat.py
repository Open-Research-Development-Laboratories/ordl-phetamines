...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "ordlctl chat session before final synthesis."
        )
    )
    parser.add_argument(
        "--roles",
    ...
    parser.add_argument(
        "--session-id",
        default=None,
        help="Optional ordlctl sessionId; defaults to most recently active session.",
    )
    parser.add_argument(
        "--max-chars",
        type=int,
    ...
    status("[3/4] Staging worker dumps to ordlctl chat...")
