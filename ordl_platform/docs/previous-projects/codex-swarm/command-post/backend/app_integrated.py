...
try:
    ...
    from training.api import training_bp, init_training_api
...
try:
    ...
    from agents.api import agents_bp, init_agents_api
    AGENTS_AVAILABLE = True
    print("[OK] Agent module loaded")
except Exception as e:
    AGENTS_AVAILABLE = False
    print(f"[WARN] Agent module unavailable: {e}")

# RAG Module
try:
    from rag.vector_kb import get_knowledge_base
    from rag.api import rag_bp, init_rag_api
    RAG_AVAILABLE = True
...
try:
    ...
    print("[OK] Red Team module loaded")
except Exception as e:
    REDTEAM_AVAILABLE = False
    print(f"[WARN] Red Team module unavailable: {e}")

# Blue Team Module
try:
    from blueteam import get_blueteam_manager
    from blueteam.api import blueteam_bp, init_blueteam_api
    BLUETEAM_AVAILABLE = True
    print("[OK] Blue Team module loaded")
except Exception as e:
    BLUETEAM_AVAILABLE = False
    print(f"[WARN] Blue Team module unavailable: {e}")

# Initialize Red Team
redteam_manager = None
if REDTEAM_AVAILABLE and audit_logger:
    try:
        init_redteam_database()
        redteam_manager = get_redteam_manager(audit_logger)
        init_redteam_api(redteam_manager)
        print("[SECURITY] Red Team API initialized")
    except Exception as e:
        print(f"[SECURITY ERROR] Red Team init failed: {e}")

# Initialize Blue Team
blueteam_manager = None
if BLUETEAM_AVAILABLE:
    try:
        blueteam_manager = get_blueteam_manager()
        init_blueteam_api(blueteam_manager)
        print("[SECURITY] Blue Team API initialized")
    except Exception as e:
        print(f"[SECURITY ERROR] Blue Team init failed: {e}")
...
if RAG_AVAILABLE:
    try:
        ...
        print(f"[RAG ERROR] Knowledge base init failed: {e}")
        RAG_AVAILABLE = False
...
if TRAINING_AVAILABLE:
    try:
        training_manager = get_trainer()
        init_training_api(training_manager)
        print("[TRAINING] Training pipeline initialized")
    except Exception as e:
        print(f"[TRAINING ERROR] Training init failed: {e}")
        TRAINING_AVAILABLE = False
...
if AGENTS_AVAILABLE:
    try:
        ...
        init_agents_api(agent_manager)
        print("[AGENTS] Agent system initialized")
    except Exception as e:
        print(f"[AGENTS ERROR] Agent init failed: {e}")
        AGENTS_AVAILABLE = False
...
if REDTEAM_AVAILABLE and 'redteam_bp' in dir():
    try:
        app.register_blueprint(redteam_bp)
        log_system_event('info', 'security', 'Red Team blueprint registered')
    except Exception as e:
        log_system_event('error', 'security', f'Red Team blueprint registration failed: {e}')

# Register Blue Team blueprint
if BLUETEAM_AVAILABLE and 'blueteam_bp' in dir():
    try:
        app.register_blueprint(blueteam_bp)
        log_system_event('info', 'security', 'Blue Team blueprint registered')
    except Exception as e:
        log_system_event('error', 'security', f'Blue Team blueprint registration failed: {e}')

# Register RAG blueprint
try:
    if RAG_AVAILABLE:
        app.register_blueprint(rag_bp)
        log_system_event('info', 'rag', 'RAG blueprint registered')
        print("[RAG] Blueprint registered")
except Exception as e:
    log_system_event('error', 'rag', f'RAG blueprint registration failed: {e}')
    print(f"[RAG ERROR] Blueprint registration failed: {e}")

# Register Training blueprint
try:
    if TRAINING_AVAILABLE:
        app.register_blueprint(training_bp)
        log_system_event('info', 'training', 'Training blueprint registered')
        print("[TRAINING] Blueprint registered")
except Exception as e:
    log_system_event('error', 'training', f'Training blueprint registration failed: {e}')
    print(f"[TRAINING ERROR] Blueprint registration failed: {e}")

# Register Agent blueprint
try:
    if AGENTS_AVAILABLE:
        app.register_blueprint(agents_bp)
        log_system_event('info', 'agents', 'Agent blueprint registered')
        print("[AGENTS] Blueprint registered")
except Exception as e:
    log_system_event('error', 'agents', f'Agent blueprint registration failed: {e}')
    print(f"[AGENTS ERROR] Blueprint registration failed: {e}")
...
@require_auth
def list_agents():
    """List all agents from AgentManager"""
    if agent_manager:
...
@require_auth
def create_agent():
    """Create new agent using AgentManager with real LLM integration"""
    data = request.get_json()
    
    if agent_manager:
        # Use real AgentManager with LLM support
        config = {
        ...
        try:
            ...
            log_system_event('info', 'agents', f"Agent {config['name']} created with LLM support", 
...
@app.route('/api/agents/_legacy/<agent_id>', methods=['GET'])
@require_auth
def get_agent_legacy(agent_id):
    # Legacy endpoint - use /api/agents/<id> from blueprint for new agents
...
@app.route('/api/agents/_legacy/<agent_id>', methods=['DELETE'])
@require_auth
def delete_agent_legacy(agent_id):
    if agent_id not in agents_db:
        return jsonify({'error': 'Agent not found'}), 404
...
@app.route('/api/agents/_legacy/<agent_id>/message', methods=['POST'])
@require_auth
def send_agent_message_legacy(agent_id):
    """Send message to agent with LLM processing"""
    if agent_manager:
        data = request.get_json()
        ...
        try:
            result = agent_manager.send_message(agent_id, message)
            return jsonify(result)
        except Exception as e:
            logger.error(f"Error sending message to agent: {e}")
            return jsonify({'success': False, 'error': str(e)}), 500
    
    return jsonify({'success': False, 'error': 'Agent manager not available'}), 503

@app.route('/api/agents/_legacy/<agent_id>/control', methods=['POST'])
@require_auth
def control_agent_legacy(agent_id):
    """Control agent state"""
    if agent_manager:
        data = request.get_json()
